...
/**
 * Exporta todos os clientes PAGOS de uma ação para Excel
 * e marca os itens como extraídos
 */
export async function exportPaidClients(acaoId: string) {
  try {
    await requireAdmin();
    ...
    // Buscar APENAS as solicitações PAGAS dessa ação
    ...
    const paidRequests = requests.filter((r) => r.paid);

    if (paidRequests.length === 0) {
      return {
        success: false,
        error: "Nenhum cliente pago encontrado nesta ação",
      };
    }

    // Consolidar todos os itens pagos
    ...
    return {
      ...
      data: {
        base64,
        filename: `${acaoData[0].nome.replace(/[^a-zA-Z0-9]/g, "_")}-${new Date().toISOString().split("T")[0]}.xlsx`,
...
export type ExportPaidClientsResponse = Awaited<
  ReturnType<typeof exportPaidClients>
>;
