...
function getOrInitializeItemsStatus(
  ...
  if (isBulkUpload) {
    ...
    return items.map((item, index) => ({
      ...
      extracted: existingItemsStatus?.[index]?.extracted,
      extractedAt: existingItemsStatus?.[index]?.extractedAt,
    }));
  }
  ...
  return [
    {
      ...
      extracted: existingItemsStatus?.[0]?.extracted,
      extractedAt: existingItemsStatus?.[0]?.extractedAt,
    },
  ];
}

/**
 * Atualiza o status de múltiplos clientes de diferentes envios
 */
  ...
  try {
    ...
    for (const [requestId, indexes] of itemsByRequest) {
      ...
      if (requestData[0].acaoId) {
        revalidatePath(`/acoes/${requestData[0].acaoId}`);
      }
    }

    revalidatePath("/gerenciar-clientes");
    ...
    console.error("[updateClientsStatus] Error:", error);
    return {
      success: false,
      error: "Erro ao atualizar status dos clientes",
    };
  }
}
